pub mod functions {
    ...
    use crate::utils::utils::{get_channel_by_name_rgb_color, get_channel_by_name_rgba_u8};
    ...
    pub fn bloom(
        ...
        output.enumerate_pixels_mut().for_each(|(x, y, pixel)| {
            ...
            let (r, g, b) = (
                ((lhs.0 as f32) + (lhs.0 as f32 * rhs.0 as f32 / 255.0) * intensity).min(255.0)
                    as u8,
                ((lhs.1 as f32) + (lhs.1 as f32 * rhs.1 as f32 / 255.0) * intensity).min(255.0)
                    as u8,
                ((lhs.2 as f32) + (lhs.2 as f32 * rhs.2 as f32 / 255.0) * intensity).min(255.0)
                    as u8,
            );
